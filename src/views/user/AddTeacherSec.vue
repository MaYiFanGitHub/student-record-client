  ...
  <div>
    ...
    <div class="user-add-content">
      ...
      >
        ...
        <el-table-column label="职称" min-width="170" align="center">
          <template slot-scope="scope">
            <el-select
              v-model="scope.row.teacher_title"
              placeholder="请选择"
              style="width:100%"
            >
...
import { addMoreTeacher } from "@api/user";
export default {
  mounted() {
  ...
  methods: {
    ...
    async addMoreUser() {
      const result = await addMoreTeacher(this.userList);
      if (result) {
        this.$message({
          type: "success",
          message: "添加成功!"
        });
        this.userList = [{ role_id: 4 }];
      }
    },
    // 读取文件
